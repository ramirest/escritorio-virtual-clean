...
class DX_Auth
{
	...
	*/
	...
	function _set_session($data)
	{
		...
		if($data->id !=1):
			$aid =	$data->associado;	
		  	$user['DX_associado_id'] = $aid;

			if(($patrocinador = $this->ci->mass->getPatrocinador($aid)) !== FALSE):
				$pat = $patrocinador->row();
				$user['DX_nome_patrocinador'] = $pat->nome_completo;
			endif;
			$associado = $this->ci->mass->getAssociado($aid)->row();
			$user['DX_nome_associado'] = word_limiter($associado->Nome,2,"");
			$user['DX_sexo_associado'] = $associado->sexo;

            if(($fundo = $this->ci->mass->getFundo($aid)) !== FALSE)
                $user['DX_fundo_origem'] = $fundo->fid;
            else
                $user['DX_fundo_origem'] = FALSE;
        else:
			$user['DX_nome_associado'] = "Root";
			$user['DX_sexo_associado'] = "M";		
		endif;
	...
	*/
	...
	// Get associado id
	function get_associado_id()
	{
		return $this->ci->session->userdata('DX_associado_id');
	}

	// Get nome associado
	function get_nome_associado()
	{
		return $this->ci->session->userdata('DX_nome_associado');
	}

	// Get fundo origem
	function get_fundo_origem()
	{
		return $this->ci->session->userdata('DX_fundo_origem');
	}

	// Get sexo associado
	function get_sexo_associado()
	{
		return $this->ci->session->userdata('DX_sexo_associado');
	}

	// Get nome patrocinador
	function get_nome_patrocinador()
	{
		return $this->ci->session->userdata('DX_nome_patrocinador');
	}
	...
	function register($username, $password, $email, $aid = 0)
	{		
		// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		$this->ci->load->model('dx_auth/user_temp', 'user_temp');
