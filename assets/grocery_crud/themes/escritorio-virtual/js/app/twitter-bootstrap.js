...
$(function(){
	...
	$('.delete-row').live('click', function() {
        ...
        var modal_content = '<div id="dialog_modal_message" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="dialog_modal_message_label" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><div class="modal-header">	<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>	<h3 id="dialog_modal_message_label"> Confirmação </h3></div><div class="modal-body">	<p>'+ message_alert_delete + '</p></div><div class="modal-footer">	<button class="btn cancel-confirmation" data-dismiss="modal" aria-hidden="true">Cancelar</button>	<button class="btn btn-primary ok-confirmation">Ok</button></div></div></div></div>';
        $('#ajax_list').after(modal_content);

        $('#dialog_modal_message')
            .modal({ keyboard: false });
        $('#dialog_modal_message').find('button.ok-confirmation').click(function(){
                deleteGroceryCrudInformation($(this).data('target-url'));
...
function confirmationModalDialog(title_modal, message_text){
	...
	var modal_content = '<div id="dialog_modal_message" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="dialog_modal_message_label" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><div class="modal-header">	<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>	<h3 id="dialog_modal_message_label">' + title_modal + '</h3></div><div class="modal-body">	<p>'+ message_text + '</p></div><div class="modal-footer">	<button class="btn cancel-confirmation" data-dismiss="modal" aria-hidden="true">Cancel</button>	<button class="btn btn-primary ok-confirmation">Ok</button></div></div></div></div>';
	$('#ajax_list').after(modal_content);

	$('#dialog_modal_message')
		.modal({ keyboard: false });
    $('#dialog_modal_message').on('show', function(){
...
function deleteGroceryCrudInformation(delete_url){
	$('#ajax-loading').removeClass('hide');
	$.post(delete_url, {'csrf_s': $(':input[name="csrf_s"]').val() }, function(data){
		if(data.success) {
			$('#filtering_form').trigger('submit');
